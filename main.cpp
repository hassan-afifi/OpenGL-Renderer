...
int main( int argc, char* args[] )
{
	...
	win = SDL_CreateWindow( "Hello SDL&OpenGL!",		// window title
							1280,						// window width
							720,						// window height
							SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE);			// display properties
...

	{
		...
		while (!quit)
		{
			...
			while ( SDL_PollEvent(&ev) )
			{
				...
				switch (ev.type)
				{
					...
					case SDL_EVENT_KEY_DOWN:
						...
						if ( ( ev.key.key == SDLK_RETURN )  // Enter was pressed, ...
							 && ( ev.key.mod & SDL_KMOD_ALT )   // together with the Alt key, ...
							 && !( ev.key.mod & ( SDL_KMOD_SHIFT | SDL_KMOD_CTRL | SDL_KMOD_GUI ) ) ) // but other modifier keys were not pressed.
						{
						...
						if ( ( ev.key.key == SDLK_F1 )  // F1 was pressed, ...
							 && ( ev.key.mod & SDL_KMOD_CTRL )   // together with the CTRL key, ...
							 && !( ev.key.mod & ( SDL_KMOD_SHIFT | SDL_KMOD_ALT | SDL_KMOD_GUI ) ) ) // but other modifier keys were not pressed.
						{
							ShowImGui = !ShowImGui;
			...
			static Uint64 LastTick = SDL_GetTicks(); // we store what the previous "tick" was in a static variable.
			Uint64 CurrentTick = SDL_GetTicks(); // What is the current "tick".
			...
			ImGui_ImplSDL3_NewFrame(); // After this, any imgui function can be called, right up to ImGui::Render().
